...
int main( int argc, char* argv[] )
{
	...
	while( 1 )
	{
		...
		//Go through connections and set to correct file descriptor set and update nfds
		for(std::vector<ConnectionData>::iterator it = connections.begin(); it != connections.end(); ++it) {
                        if (it->state == eConnStateReceiving) {
				FD_SET(it->sock, &readfds);
                        } else if (it->state == eConnStateSending) {
		...
		// Call select to handle fds
		int retval = select(nfds, &readfds, &writefds, NULL, NULL);
		...
		// If new connection, listening to standard read port
		...
		// Go through all connections, process them depending on read/write and close if necessary
		for(std::vector<ConnectionData>::iterator it = connections.begin(); it != connections.end(); ++it) {
			bool processFurther = true;

			// Check if read or write and call correct function depending
			if (FD_ISSET(it->sock, &readfds)) {
				processFurther = process_client_recv(*it);
			} else if (FD_ISSET(it->sock, &writefds)) {
		...
		connections.erase(std::remove_if(connections.begin(), connections.end(), &is_invalid_connection), connections.end());
	}
